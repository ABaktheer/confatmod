...
#                     np.savetxt(r'e:\\Models_implementation\python_results\U111_loaded_slip2.txt', np.transpose(np.vstack((U_record, U_k))), delimiter=" ", fmt="%s")
#
#                     np.savetxt(r'e:\\Models_implementation\python_results\F2222222.txt', np.transpose(
# np.vstack((F_record, F_int))), delimiter=" ", fmt="%s")
                    break
                k += 1
                step_flag = 'corrector'
...
# BCDof(var='u', dof=n_dofs - 1, value=2.5, time_function=tf)]
    ...
    ts.bc_list = [BCDof(var='u', dof=0, value=0.0),
                  BCDof(var='f', dof=n_dofs - 1, value=500)]
