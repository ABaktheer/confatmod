...
class Pfanner_fatigue_model():
    ...
    def get_Nf(self, sig_max, sig_min, T):
    ...
    def get_N(self, D, sig_max, sig_min, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_0):

        N_f = self.get_Nf(sig_max, sig_min, T)

        d = self.get_fatigue_response(
            np.int(N_f), sig_max, sig_min, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, np.int(N_f / 200.0), N_0)

        if D - d[1][-1] > 0.05:
            print 'maximum fatigue damage has been reached'

        return d[0][np.abs(d[1] - D).argmin()]

    def get_damage_variable_amplitude(self, N_1, N_2, N_3,
                                      sig_max_1, sig_max_2, sig_max_3,
                                      sig_min_1, sig_min_2, sig_min_3,
                                      T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_0):
        ...
        d_2 = self.get_fatigue_response(
            N_2 + N_2_eq, sig_max_2, sig_min_2, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_2_eq)

        N_3_eq = self.get_N(
            d_2[1][-1], sig_max_3, sig_min_3, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)

        d_3 = self.get_fatigue_response(
            N_3 + N_3_eq, sig_max_3, sig_min_3, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_3_eq)

        d_arr_1 = np.vstack((d_1[1][1:], d_2[1][1:], d_3[1][1:]))
        d_arr_2 = np.vstack((d_1[1][1:], d_2[1][2:], d_3[1][2:]))
        n_arr_1 = np.vstack((d_1[0][1:], d_2[0][1:], d_3[0][1:]))
        n_arr_2 = np.vstack(
            (d_1[0][1:], d_2[0][2:] + (d_1[0][-1] - N_2_eq), (d_3[0][2:] + (d_2[0][-1] + d_1[0][-1] - N_2_eq - N_3_eq))))

        return n_arr_1, d_arr_1, n_arr_2,  d_arr_2

    def get_damage_repeated_blocks(self, r, N_1, N_2, N_3, N_4, N_5,
                                   sig_max_1, sig_max_2, sig_max_3, sig_max_4, sig_max_5,
                                   sig_min_1, sig_min_2, sig_min_3, sig_min_4, sig_min_5,
                                   T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_0):

        d_arr_rep_1 = np.zeros(1)
        d_arr_rep_2 = np.zeros(1)
        n_arr_rep_1 = np.zeros(1)
        n_arr_rep_2 = np.zeros(1)
        ...
        N_4_eq = self.get_N(
            d_3[1][-1], sig_max_4, sig_min_4, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)

        d_4 = self.get_fatigue_response(
            N_4 + N_4_eq, sig_max_4, sig_min_4, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_4_eq)

        N_5_eq = self.get_N(
            d_4[1][-1], sig_max_5, sig_min_5, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)

        d_5 = self.get_fatigue_response(
            N_5 + N_5_eq, sig_max_5, sig_min_5, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_5_eq)

        d_arr_1 = np.vstack(
            (d_1[1][1:], d_2[1][1:], d_3[1][1:], d_4[1][1:], d_5[1][1:]))
        d_arr_2 = np.vstack(
            (d_1[1][1:], d_2[1][2:], d_3[1][2:], d_4[1][2:], d_5[1][2:]))
        n_arr_1 = np.vstack(
            (d_1[0][1:], d_2[0][1:], d_3[0][1:], d_4[0][1:], d_5[0][1:]))
        n_arr_2 = np.vstack(
            (d_1[0][1:], d_2[0][2:] + (d_1[0][-1] - N_2_eq),
             (d_3[0][2:] + (d_2[0][-1] + d_1[0][-1] - N_2_eq - N_3_eq)),
             (d_4[0][2:] + (d_3[0][-1] + d_2[0][-1] + d_1[0]
                            [-1] - N_2_eq - N_3_eq - N_4_eq)),
             (d_5[0][2:] + (d_4[0][-1] + d_3[0][-1] + d_2[0][-1] + d_1[0][-1] - N_2_eq - N_3_eq - N_4_eq - N_5_eq))))

        i = 1
        for i in range(1, r):

            N_1_eq = self.get_N(
                d_arr_1[-1], sig_max_1, sig_min_1, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)

            print 'N_1_eq ------------->', N_1_eq

            d_1 = self.get_fatigue_response(
                N_1 + N_1_eq, sig_max_1, sig_min_1, T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_1_eq)
            ...
            d_arr_1 = np.vstack(
                (d_arr_1, d_1[1][1:], d_2[1][1:], d_3[1][1:], d_4[1][1:], d_5[1][1:]))
            d_arr_2 = np.vstack(
                (d_arr_2, d_1[1][1:], d_2[1][2:], d_3[1][2:], d_4[1][2:], d_5[1][2:]))
            n_arr_1 = np.vstack(
                (n_arr_1, d_1[0][1:], d_2[0][1:], d_3[0][1:], d_4[0][1:], d_5[0][1:]))

            n_arr_2 = np.vstack(
                (n_arr_2, d_1[0][1:] - N_1_eq + n_arr_2[-1], d_2[0][2:] + (d_1[0][-1] - N_2_eq) - N_1_eq + n_arr_2[-1],
                 (d_3[0][2:] +
                  (d_2[0][-1] + d_1[0][-1] - N_2_eq - N_3_eq) - N_1_eq + n_arr_2[-1]),
                 (d_4[0][2:] + (d_3[0][-1] + d_2[0][-1] + d_1[0]
                                [-1] - N_2_eq - N_3_eq - N_4_eq) - N_1_eq + n_arr_2[-1]),
                 (d_5[0][2:] + (d_4[0][-1] + d_3[0][-1] + d_2[0][-1] + d_1[0][-1] - N_2_eq - N_3_eq - N_4_eq - N_5_eq) - N_1_eq + n_arr_2[-1])))
            #i = i + 1


#         d_arr_rep_1 = np.vstack((d_arr_rep_1, d_arr_1))
#         d_arr_rep_2 = np.vstack((d_arr_rep_2, d_arr_2))
#         n_arr_rep_1 = np.vstack((n_arr_rep_1, n_arr_1))
#         n_arr_rep_2 = np.vstack((n_arr_rep_2, n_arr_2))

        #i = i + 1

#         N_arr = np.array([N_1, N_2, N_3, N_4, N_5])
#         N_eq_arr = np.zeros(5, dtype=np.int_)
#
#         sigma_max_arr = np.array([
#             sig_max_1, sig_max_2, sig_max_3, sig_max_4, sig_max_5])
#         sigma_min_arr = np.array([
#             sig_min_1, sig_min_2, sig_min_3, sig_min_4, sig_min_5])
#
#         d_arr_1 = self.get_fatigue_response(
#             N_arr[0], sigma_max_arr[0], sigma_min_arr[0], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)[1]
#         d_arr_2 = self.get_fatigue_response(
#             N_arr[0], sigma_max_arr[0], sigma_min_arr[0], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)[1]
#         n_arr_1 = self.get_fatigue_response(
#             N_arr[0], sigma_max_arr[0], sigma_min_arr[0], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)[0]
#
#         N_eq_arr[0] = self.get_N_2(
#             d_arr_1[-1], sigma_max_arr[1], sigma_min_arr[0], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)
#
#         print 'N_eq_arr[0]', N_eq_arr[0]
#
#         for i in range(1, r):
#             d_arr_1 = np.vstack((d_arr_1, self.get_fatigue_response(
#                 N_arr[i] + N_eq_arr[i - 1], sigma_max_arr[i], sigma_min_arr[i], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_eq_arr[i - 1])[1][1:]))
#             d_arr_2 = np.vstack((d_arr_2, self.get_fatigue_response(
#                 N_arr[i] + N_eq_arr[i - 1], sigma_max_arr[i], sigma_min_arr[i], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_eq_arr[i - 1])[1][2:]))
#             n_arr_1 = np.vstack((n_arr_1, self.get_fatigue_response(
#                 N_arr[i] + N_eq_arr[i - 1], sigma_max_arr[i], sigma_min_arr[i], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, N_eq_arr[i - 1])[0][1:]))
#
#             N_eq_arr[i] = self.get_N_2(
#                 d_arr_1[-1], sigma_max_arr[i + 1], sigma_min_arr[i + 1], T, leq, Gcl, Ec, fc, eps_c, b, xci_c, inc, 1)
#             #N_eq_arr = N_eq_arr[i]
#
#         n_arr_2 = np.zeros_like(d_arr_2)

        # return n_arr_rep_1, d_arr_rep_1, n_arr_rep_2,  d_arr_rep_2
        return n_arr_1, d_arr_1, n_arr_2,  d_arr_2
...
if __name__ == '__main__':
    ...
    sig_min_2 = - S_min_2 * fc
    sig_min_3 = - S_min_3 * fc
    sig_min_4 = - S_min_4 * fc
    sig_min_5 = - S_min_5 * fc

#     d_asc = P.get_damage_variable_amplitude(
#         N_1, N_2, N_3, sig_max_1, sig_max_2, sig_max_3, sig_min_1, sig_min_2, sig_min_3,
#         0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc, 1)

#     d_des = P.get_damage_variable_amplitude(
#         N_3, N_2, 2000, sig_max_3, sig_max_2, sig_max_1, sig_min_3, sig_min_2, sig_min_1,
#         0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc, 1)

    d_des = P. get_damage_repeated_blocks(10, N_1, N_2, N_3, N_4, N_5, sig_max_1, sig_max_2, sig_max_3, sig_max_4, sig_max_5,
                                          sig_min_1, sig_min_2, sig_min_3, sig_min_4, sig_min_5,
                                          0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc, 1)

    N = 100000
#     D_1 = P.get_fatigue_response(
#         N,  sig_max_1, sig_min_1, 0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc * 1, 1)
#     D_2 = P.get_fatigue_response(
#         N,  sig_max_2, sig_min_1, 0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc * 5, 1)
#     D_3 = P.get_fatigue_response(
#         N,  sig_max_3, sig_min_3, 0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc * 10, 1)
#     D_4 = P.get_fatigue_response(
#         N,  sig_max_4, sig_min_4, 0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75, 0.2, inc * 50, 1)
#     D_5 = P.get_fatigue_response(
# N,  sig_max_5, sig_min_5, 0.1, 0.25, 0.03, 40000.0, 40.0, 0.0022, 0.75,
# 0.2, inc * 100, 1)
    ...
    #plt.plot(D_3[0][1:], D_3[1][1:], '--k')
    # plt.xlabel('N')
    # plt.ylabel('damage')

    plt.subplot(111)
    plt.plot(d_des[0][2:], d_des[1][2:], 'k')
    plt.plot(d_des[2][2:], d_des[3][2:], 'r')
#     plt.plot(D_1[0][1:], D_1[1][1:], '--k')
#     plt.plot(D_2[0][1:], D_2[1][1:], ':k')
#     plt.plot(D_3[0][1:], D_3[1][1:], '--k')
#     plt.plot(D_4[0][1:], D_4[1][1:], ':k')
#     plt.plot(D_5[0][1:], D_5[1][1:], '--k')
    plt.xlabel('N')
    plt.ylabel('damage')
