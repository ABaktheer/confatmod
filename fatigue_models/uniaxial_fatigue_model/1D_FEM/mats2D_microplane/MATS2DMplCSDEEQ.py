...
from vectorized_fem.mats2D_microplane.vmats2D_eval_2 import MATS2D
...
class MATSXDMicroplaneDamageJir(MATS2DEval, MATS2D):
    ...
    def _get_e_N_Emn(self, e_Emna):
        ...
        e_N_Emn = einsum('na, Emna->Emn',  self._MPN, e_Emna)
        return e_N_Emn
    ...
    def _get_phi_Emab(self, kappa_Emn):
        ...
        print 'omega', 1.0 - phi_Emn[:, 0, :]
        # integration terms for each microplanes
        phi_Emab = einsum('Emn,nab,n ->Emab', phi_Emn,  self._MPNN, self._MPW)
        print 'phi_Emab', phi_Emab[:, 0, :, :]

#         phi_mtx_arr = array([phi_Emn[i] * self._MPNN[i, :, :] * self._MPW[i]
#                              for i in range(0, self.n_mp)])
#         # sum of contributions from all microplanes
#         # sum over the first dimension (over the microplanes)
#         phi_Emab = phi_mtx_arr.sum(0)
#         print 'phi_Emab', phi_Emab[:, 0, :, :]
    ...
    def _get_beta_Emabcd(self, phi_Emab):
        ...
        # print 'beta_Emijkl', beta_Emijkl[:, 0, :, :, :, :]
        return beta_Emijkl
