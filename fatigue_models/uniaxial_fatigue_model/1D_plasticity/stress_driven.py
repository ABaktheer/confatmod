...
def get_stress_strain(sig_arr, sig_0, E, K, gamma):
...
if __name__ == '__main__':
    ...
    s_levels_2.reshape(-1, 2)[:, 1] = 20
    s_levels_2[0] = 0
    s_history_2 = s_levels_2.flatten()

    # slip array as input
    sig_arr_2 = np.hstack([np.linspace(s_history_2[i], s_history_2[i + 1], 1000)
                           for i in range(len(s_history_2) - 1)])

    eps_arr_1, sig_arr_1, eps_p_arr_1 = get_stress_strain(
        sig_arr_1, sig_0=10, K=10000, gamma=2000, E=30000)
    eps_arr_2, sig_arr_2, eps_p_arr_2 = get_stress_strain(
        sig_arr_2, sig_0=10, K=10000, gamma=2000, E=30000)
