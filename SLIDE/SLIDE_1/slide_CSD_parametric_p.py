...
if __name__ == '__main__':
    ...
    s_levels = linspace(0,  -60, 2)
    s_levels[0] = 0
    s_levels.reshape(-1, 2)[:, 0] *= -1
    s_history_3 = s_levels.flatten()

    eps_arr_1 = hstack([linspace(s_history_1[i], s_history_1[i + 1], 100)
                      for i in range(len(s_history_1) - 1)])

    
    sig_arr = hstack([linspace(s_history_3[i], s_history_3[i + 1], 100)
                      for i in range(len(s_history_3) - 1)])


    eps_arr_1, tau_arr_1, w_arr_1, xs_pi_arr_1, xs_pi_cum_arr_1 = get_CSD_Law(
        eps_arr_1, sig_arr,, tau_pi_bar=4, K=50, gamma=10000, G=14166, S=.00004, c=-1.0, r=1.0, m=0.0, p=0)

    eps_arr_2, tau_arr_2, w_arr_2, xs_pi_arr_2, xs_pi_cum_arr_2 = get_CSD_Law(
        eps_arr_1, sig_arr, tau_pi_bar=4, K=50, gamma=10000, G=14166, S=.00004, c=1.0, r=1.0, m=0.0,  p=1)

    ax1 = plt.subplot(221)
    ax1.plot(eps_arr_1, tau_arr_1, 'k')
    ax1.plot(eps_arr_2, tau_arr_2, 'r')
    ...
    plt.plot(eps_arr_1, w_arr_1, 'k')
    plt.plot(eps_arr_1, w_arr_2, 'r')
...
#     plt.plot(xs_pi_cum_arr_6[:-2], w_arr_6[:-2], 'r')
    ...
    plt.plot(sig_arr[:-2], w_arr_1[:-2], 'k')
    plt.plot(sig_arr[:-2], w_arr_2[:-2], 'r')
#     plt.plot(xs_pi_cum_arr_3[:-2], w_arr_3[:-2], 'r')
#     plt.plot(xs_pi_cum_arr_4[:-2], w_arr_4[:-2], 'r')
#     plt.plot(xs_pi_cum_arr_5[:-2], w_arr_5[:-2], 'r')
#     plt.plot(xs_pi_cum_arr_6[:-2], w_arr_6[:-2], 'r')
