...
def get_bond_slip_(eps_N_arr, eps_T_arr, sig_0, E_T, m, S_T, c_T, r, K_T, gamma_T, sig_N):
    ...
    for i in range(1, len(eps_N_arr)):
        ...
        sig_N_i_eff_trial = sig_N
        sig_T_i_eff_trial  = E_T* (eps_T_i - eps_T_p_i)
        
        
        f_trial = np.fabs(sig_T_i_eff_trial - gamma_T * alpha_T_i)  -\
         (sig_0  + K_T * z_T_i - m * sig_N) 
         
        if f_trial > 1e-8: 
            
            def f(vars):
                
                delta_lamda , eps_T_p, z_T,  alpha_T, omega_T = vars
                
                f1 = sig_0 + K_T * z_T - m * sig_N
                ...
                f_lamda  = np.fabs(sig_T_i_eff_trial - gamma_T * alpha_T_i) - delta_lamda * (E_T/(1 - omega_T) + gamma_T)  - f1 
                ...
                f_omega_T = omega_T - omega_T_i  - delta_lamda* (1 -omega_T)**c_T * (Y_T / S_T)**r
                

                

                return [f_lamda, f_eps_T_p, f_z_T, f_alpha_T, f_omega_T]
            
            x0 = np.array([0, 0, 0, 0, 0])
            sol   =  root(f, x0 = x0,  method='lm')
            delta_lamda, eps_T_p_i, z_T_i,alpha_T_i, omega_T_i = sol.x
    ...
    return sig_T_trial_arr,  sig_T_arr, eps_T_p_arr, omega_T_arr, f_arr 
...
if __name__ == '__main__':
    ...
    sig_T_trial_arr, sig_T_arr, eps_T_p_arr, omega_T_arr, f_arr = get_bond_slip_(
        eps_N_arr, eps_T_arr, sig_0, E_T, m, S_T, c_T, r, K_T, gamma_T, sig_N)
