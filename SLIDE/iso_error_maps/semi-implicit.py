...
Version: two dimensional SLIDE (semi implicit return mapping)
'''
    ...
    for i in range(1, len(eps_N_arr)):
        ...
        if f_trial > 1e-8: 
            ...
            def f(vars):
                ...
                delta_lamda ,   sig_N_eff  , sig_T_eff = vars
                ...
                f_T = sig_T_eff - sig_T_i_eff_trial +  delta_lamda * E_T * np. sign(sig_T_i_eff_trial - gamma_T * alpha_T_i)  /(1. - omega_T_i)
                
                return [f_lamda, f_N, f_T]
            
            x0 = np.array([0, 0, 0])
            
            sol   =  root(f, x0 = x0,  method='lm')
            delta_lamda,  sig_N_eff,  sig_T_eff = sol.x
...
d_eps_N_arr = np.linspace(0.0, d_eps_N, 5)
d_eps_T_arr = np.linspace(0.0, d_eps_T, 5)
...
print(Z.shape)
fig, ax = plt.subplots()
CS = ax.contour(X, Y, Z, 15,  cmap='inferno')
