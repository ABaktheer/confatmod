...
two dimensional SLIDE (with reduced 4 nonlinear equations)
'''
    ...
    for i in range(1, len(eps_N_arr)):
        ...
        if f_trial > 1e-8: 
            ...
            def f(vars):
                ...
                delta_lamda ,  sig_N_eff, sig_T_eff, omega_N, omega_T  = vars
                ...
                Y_N = 0.5 * E_N * (eps_N_i - (eps_N_p_i + delta_lamda * N  /(1. - omega_N)))**2.0
                Y_T = 0.5 * E_T * (eps_T_i - (eps_T_p_i + delta_lamda * sign / (1.0 - omega_T)))**2.0
                
                f_lamda  = np.fabs(sig_T_eff - gamma_T * (alpha_T_i + delta_lamda ))  - f1 * ft
                
                f_N = sig_N_eff - sig_N_i_eff_trial +  delta_lamda * E_N * N  /(1. - omega_N)
                f_T = sig_T_eff - sig_T_i_eff_trial +  delta_lamda * E_T * np.sign(sig_T_i_eff_trial - gamma_T * alpha_T_i) /(1. - omega_T)
                ...
                return [f_lamda, f_N, f_T, f_omega_N, f_omega_T]
            
            x0 = np.array([0, 0, 0, 0, 0])
            
            sol   =  root(f, x0 = x0,  method='lm')
            delta_lamda,  sig_N_eff, sig_T_eff, omega_N, omega_T = sol.x
            

            eps_N_p_i +=  delta_lamda* (m  - np.heaviside((sig_N_eff), 1) * ((m * sig_N_eff **2)/ (sig_t)**2 - (sig_0  + K_T * (z_T_i + delta_lamda)  - m * sig_N_eff) * (2 * sig_N_eff / (sig_t)**2 ) )) /(1. - omega_N)
            eps_T_p_i += delta_lamda * np.sign(sig_T_i_eff_trial - gamma_T * alpha_T_i) / (1.0 - omega_T)
            ...
            sig_T_i = (1.0 - omega_T) * sig_T_eff
            
            omega_N_i = omega_N
            omega_T_i = omega_T
            
            f = np.fabs(sig_T_i /(1 - omega_T)  - gamma_T * alpha_T_i)  -\
            (sig_0  + K_T * z_T_i - m * sig_N_i /(1 - omega_N)) * (1.0 - np.heaviside(( sig_N_i /(1 - omega_N)), 1) * (( sig_N_i /(1 - omega_N))**2 / (sig_t)**2 ))   
