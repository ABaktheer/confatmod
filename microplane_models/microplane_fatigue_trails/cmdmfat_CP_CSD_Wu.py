...
class MATSEvalMicroplaneFatigue(HasTraits):
    ...
    #-----------------------------------------------
    # anistropy control parameter - [Wu]
    #-----------------------------------------------
...
#         return new_sctx
    ...
    #-------------------------------------------------------------------------
    # microplane constitutive law (Tangential)-(Pressure sensitive cumulative damage)
    #-------------------------------------------------------------------------
...
#             + G0 * einsum('ijmn,mnrs,rskl -> ijkl', I_dev_ijkl, M_dev_ijkl, I_dev_ijkl)\
    ...
    def get_corr_pred(self, sctx, eps_app_eng, sigma_kk):
        ...
        S_ijkl = self._get_S_3_tns(sctx, eps_app_eng, sigma_kk)
...
if __name__ == '__main__':
    ...
    m = 0.0
    n = 1.0
    eps_1 = array([array([[n * s_arr_1[i], 0, 0],
                          [0, m * s_arr_1[i], 0],
                          [0, 0, 0]]) for i in range(0, len(s_arr_1))])

    eps_2 = array([array([[n * s_arr_2[i], 0, 0],
                          [0,  m * s_arr_2[i], 0],
    ...
    plt.plot(eps_1[:, 1, 1], sigma_1[:, 1, 1], linewidth=1, label='sigma_22')
