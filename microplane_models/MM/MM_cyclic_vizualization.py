...
#     plt.show()
    ...
    filename6 = os.path.join(out_path, 'colorbar' + '.pdf')
    ...
    for i in peaks:  # loop over the minima and maxima at cyclic loading
        ...
        # creates figure window, 1000x1000 pixels
        ...
        idx = np.where(normals2[:, 2] > 0)
        value = np.concatenate((w_2_T[i], w_2_T[i]))

        f_pipe3 = m.points3d(normals2[idx[:], 0], normals2[idx[:], 1],
                             origin_coord[idx[:], 2], value[idx[:]].reshape(1, 28), colormap='Reds', mode='sphere', resolution=100, scale_factor=0.15 * factor, scale_mode='none')
        ...
        filename1 = os.path.join(
            out_path, 'x,y' + 'animation' + np.str(i) + '.png')
        ...
        idx = np.where(normals2[:, 1] > 0)

        f_pipe3 = m.points3d(normals2[idx[:], 0], origin_coord[idx[:], 1],
                             normals2[idx[:], 2], value[idx[:]].reshape(1, 28), colormap='Reds', mode='sphere', resolution=100, scale_factor=0.15 * factor, scale_mode='none')
        ...
        filename2 = os.path.join(
            out_path, 'x,z' + 'animation' + np.str(i) + '.png')
        ...
        idx = np.where(normals2[:, 0] > 0)

        f_pipe3 = m.points3d(origin_coord[idx[:], 0], normals2[idx[:], 1],
                             normals2[idx[:], 2], value[idx[:]].reshape(1, 28), colormap='Reds', mode='sphere', resolution=100, scale_factor=0.15 * factor, scale_mode='none')
        ...
        filename3 = os.path.join(
            out_path, 'y,z' + 'animation' + np.str(i) + '.png')

        m.savefig(filename=filename3)
        m.close()

        filename4 = os.path.join(
            out_path, 'comb' + 'animation' + np.str(i) + '.pdf')
        filename5 = os.path.join(
            out_path, 'comb' + 'animation' + np.str(i) + '.png')
...
#         image_names = [filename1, filename2, filename3]
#         images = []
...
#             final_image[:image.shape[0], current_x:image.shape[1] +
#                         current_x, :] = image
#             current_x += image.shape[1]
#
#         cv2.imwrite(filename5, final_image)
