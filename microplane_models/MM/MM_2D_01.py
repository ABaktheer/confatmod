...
#         z_N = - w_N * thres_2
    ...
    def get_tangential_Law(self, e_T, sctx, sigma_kk):
        ...
        #eps_T_pi_cum = eps_T_pi_cum + delta_lamda / (1 - w_T[:,])
        eps_T_pi_cum = eps_T_pi_cum

        z_T = z_T + delta_lamda

        new_sctx = zeros((self.n_mp, 6))
        new_sctx[:, 0] = w_T
        new_sctx[:, 1] = z_T
...
if __name__ == '__main__':
    ...
    s_levels = linspace(0, -0.015, 2000)
    #s_levels[0] = 0
    eps_max = -0.001
    eps_min = -0.0025
    ...
    w_1_N = zeros((len(eps_1[:, 0]), n_mp))
    w_1_T = zeros((len(eps_1[:, 0]), n_mp))
    eps_P_N_1 = zeros((len(eps_1[:, 0]), n_mp))
    eps_Pi_T_1 = zeros((len(eps_1[:, 0]), n_mp, 2))
    sctx_1 = zeros((len(eps_1[:, 0]) + 1, n_mp, 11))
    ...
    s_history_2 = [-0, -0.0029, -0.00105, -
                   0.0037,  -0.00136, -0.0047, -0.00184,
                   -0.0056, -0.0022, -0.0067, -0.0028,
                   -0.0076, -0.0034, -0.0085, -0.0038, -0.0088]
    ...
    w_2_N = zeros((len(eps_2[:, 0]), n_mp))
    w_2_T = zeros((len(eps_2[:, 0]), n_mp))
    eps_P_N_2 = zeros((len(eps_2[:, 0]), n_mp))
    eps_Pi_T_2 = zeros((len(eps_2[:, 0]), n_mp, 2))
    sctx_2 = zeros((len(eps_2[:, 0]) + 1, n_mp, 11))

    for i in range(0, len(eps_2[:, 0])):
...
#              linewidth=2, label='sigma_11_(cyclic-compression)')
    ...
    for i in range(0, 28):
        plt.plot(
            eps_1[:, 0, 0], w_1_N[:, i], color='black', linewidth=1.0, label='cyclic', alpha=1)
    ...
    for i in range(0, 28):
        plt.plot(
            eps_1[:, 0, 0], w_1_T[:, i], color='black', linewidth=1.0, label='cyclic', alpha=1)
#         plt.plot(
# eps_2[:, 0, 0], w_2_T[:, i], color='red', linewidth=1.0, label='cyclic',
# alpha=1)
