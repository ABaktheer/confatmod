    ...
    for i in range(1, len(eps_N_arr)):
        ...
        if f_trial > 1e-8: 
            ...
            def f(vars):
                ...
                delta_lamda , eps_N_p, eps_T_p, z_T,  alpha_T, omega_N, omega_T = vars
                
                sig_N = (1 - omega_N) * E_N * (eps_N_i - eps_N_p)
                
                N = m/(1 - omega_N) + np.heaviside((sig_N), 1) * ((-m*sig_N**2)/((1-omega_N)**3 * (sig_t)**2 )   +\
                                                                       (sig_0  + K_T * z_T - m * sig_N/(1 - omega_N))*(2*sig_N/((1-omega_N)**2 * (sig_t)**2 ) )  )
            
                
                f1 = sig_0 + K_T * z_T - m * (sig_N_i_eff_trial - E_N * delta_lamda * N)
                
                ft =  1.0 - np.heaviside((sig_N_i_eff_trial - E_N * delta_lamda * N), 1) * ((sig_N_i_eff_trial - E_N * delta_lamda * N)**2 / (sig_t)**2  )
                ...
                f_lamda  = np.fabs(sig_T_i_eff_trial - gamma_T * alpha_T_i) - delta_lamda * (E_T/(1 - omega_T) + gamma_T)  - f1 * ft
                ...
                f_omega_N = omega_N - omega_N_i  - delta_lamda*(1 -omega_N)**c_N * (Y_N / S_N + b * Y_T /S_T) * np.heaviside((sig_N_i_eff_trial - E_N * delta_lamda * N), 1)
                f_omega_T = omega_T - omega_T_i  - delta_lamda*(1 -omega_T)**c_T * (Y_T / S_T + b * Y_N /S_N)
                

                return [f_lamda, f_eps_N_p, f_eps_T_p, f_z_T, f_alpha_T, f_omega_N, f_omega_T]
            
            x0 = np.array([0, 0, 0, 0, 0, 0, 0])
            sol   =  root(f, x0 = x0,  method='lm')
            delta_lamda, eps_N_p_i, eps_T_p_i, z_T_i,alpha_T_i, omega_N_i, omega_T_i = sol.x

          
            sig_N_i = (1 - omega_N_i) * E_N * (eps_N_i - eps_N_p_i)
            
            sig_T_i = (1 - omega_T_i) * E_T * (eps_T_i - eps_T_p_i) 
