...
#         eps_Pi_T_2[i, :, :] = sctx_2[i, :, 10:13]
    ...
    s_levels = linspace(0, -0.003, 1000)
    s_levels.reshape(-1, 2)[:, 0] = -0.004
    s_levels[0] = 0
    ...
    eps_3 = array([array([[s_arr_3[i], 0, 0],
                          [0, 0, 0],
    ...
    w_3_T = zeros(28)
    eps_P_N_3 = zeros(28)
    eps_Pi_T_3 = zeros((28, 3))
    sctx_3 = zeros((28, 13))

    sigma_3[0, :] = model.get_corr_pred(
        sctx_3[0, :], eps_3[0, :], sigma_kk_3[0])[0]

    for i in range(1, 100):

        if sigma_3[i - 1, 0, 0] < abs(sigma_max):

            eps_3[i, 0, 0] = eps_3[i - 1, 0, 0] + delta_eps
        ...
        # if sigma_3[0, 0,0]>= abs(sigma_max ):
