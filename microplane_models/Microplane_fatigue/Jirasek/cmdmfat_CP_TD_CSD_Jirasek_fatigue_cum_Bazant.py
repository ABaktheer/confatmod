...
#         w_N = w_N * thres_3 + f_w(Y_N) * thres_2
#         z_N = - w_N  # * thres_2
#
#         print 'lamda_n', f_w(Y_N)
#         new_sctx = zeros((28, 5))
#
#         new_sctx[:, 0] = w_N
#         new_sctx[:, 1] = z_N
...
#                     (self.eps_0 / eps_max[idx]) * np.exp(- (eps_max[idx] - self.eps_0) / (self.eps_f - self.eps_0)))
        ...
        w_N = 1.0 - (np.exp(-1.0 * self.A * eps_max)) * (1.0 / (1.0 + self.m *
                                                                (eps_max_cum ** self.q) +
                                                                (self.m**2) * (eps_max_cum ** (2.0 * self.q))))

        #w_N = 1.0 - (np.exp(-1.0 * self.A * eps_max))
        #dx = np.where(w_N > 1.0)
        # print idx
        #w_N[idx] = 0.99999
...
if __name__ == '__main__':
    ...
    sctx_1 = zeros((len(eps_1[:, 0, 0]) + 1, 28, 15))
    ...
    for i in range(0, len(eps_1[:, 0, 0])):
        ...
        eps_Pi_T_1[i, :, :] = sctx_1[i, :, 11:14]
        eps_Pi_T_norm_1[i, :] = sctx_1[i, :, 14]
    ...
    s_levels.reshape(-1, 2)[:, 0] = 0.00002
    #s_levels.reshape(-1, 2)[:, 1] = 0.0004
    s_levels[0] = 0
    ...
    sctx_2 = zeros((len(eps_2[:, 0, 0]) + 1, 28, 15))
    ...
    for i in range(0, len(eps_2[:, 0, 0])):
        ...
        eps_Pi_T_2[i, :, :] = sctx_2[i, :, 11:14]
        eps_Pi_T_norm_2[i, :] = sctx_2[i, :, 14]
