...
from microplane_models.SLIDE.slide_CP_CSD import get_heviside
import numpy as np
...
class MATSEvalMicroplaneFatigue(HasTraits):
    ...
    def get_normal_Law(self, eps, sctx):
        ...
        alpha_N = alpha_N + (delta_lamda / (1.0 + m * self.gamma_N * delta_lamda)) * (
            sign(sigma_n_trial - X) + m * self.gamma_N * alpha_N)
...
class MATSXDMicroplaneDamageFatigueWu(MATSEvalMicroplaneFatigue):
    ...
    def _get_phi_arr(self, sctx, eps_app_eng, sigma_kk):
        ...
        for i in range(0, self.n_mp):
            w[i] = np.maximum(w_n[i], w_T[i])    # still need to be checked
            #w[i] = w_n[i]
        phi_arr = 1. - w
...
#         phi_arr = 1. - w
    ...
    def _get_S_4_tns(self, sctx, eps_app_eng, sigma_kk):
        ...
        S_4_ijkl = (1.0 - d) * K0 * I_vol_ijkl + (1.0 - d) * G0 * I_dev_ijkl + (2.0 / 3.) * (G0 - K0) * \
            (einsum('ij,kl -> ijkl', delta, D_bar_ij) +
             einsum('ij,kl -> ijkl', D_bar_ij, delta)) + 0.5 * (- K0 + 2.0 * G0) *\
    ...
    def get_heviside(self, sigma_kk):
        if sigma_kk > 0:
            return 1.0
        else:
            return 0.0
    #-------------------------------------------------------------------------
    # Returns the fourth order secant stiffness tensor (double orthotropic N-T split) with unilateral effect
    #-------------------------------------------------------------------------

    def _get_S_6_tns(self, sctx, eps_app_eng, sigma_kk):
        ...
        H_1 = get_heviside(sigma_kk)
        H_2 = get_heviside(-sigma_kk)

        S_6_ijkl = ((1.0 - d) * H_1 + H_2) * E_N * I_vol_ijkl + (1. - d) * E_T * I_dev_ijkl + (2. / 3.) * (E_T - E_N) * \
            (einsum('ij,kl -> ijkl', delta, D_bar_ij) +
             einsum('ij,kl -> ijkl', D_bar_ij, delta)) + 0.5 * (2 * E_T - E_N) *\
            (0.5 * (einsum('ik,jl -> ijkl', delta, D_bar_ij) + einsum('il,jk -> ijkl', D_bar_ij, delta)) +
             0.5 * (einsum('il,jk -> ijkl', D_bar_ij, delta) + einsum('ik,jl -> ijkl', delta, D_bar_ij)))
...
if __name__ == '__main__':
    ...
    #s_levels.reshape(-1, 2)[:, 1] = -0.0015
    s_history_1 = s_levels.flatten()

    # n = 50  # number of increments
    s_levels_1 = linspace(-0.001, -0.012, 100)
    s_levels_2 = linspace(-0.00015, -0.006, 100)
    s_levels_1[0] = 0
    s_levels_1.reshape(-1, 2)[:, 0] = s_levels_2.reshape(-1, 2)[:, 1] * 1.5
    s_levels_1.reshape(-1, 2)[:, 1] = s_levels_1.reshape(-1, 2)[:, 1] * 1.1
    s_levels_1[0] = 0
    #s_levels.reshape(-1, 2)[:, 1] = -0.0015

    #s_levels_2 = linspace(-0.0005, -0.002, 20)

    s_history_2 = s_levels_1.flatten()
    # print s_history_2

#     maximum_loading = -0.0015
#     unloading_ratio = 0.1
#     number_of_cycles = 100
#     number_of_increments = 50
#
#     d_1 = np.zeros(1)
#     d_2 = np.linspace(0, maximum_loading, number_of_cycles * 2)
#     d_2.reshape(-1, 2)[:, 0] = maximum_loading
#     d_2.reshape(-1, 2)[:, 1] = maximum_loading * \
#         unloading_ratio
#     d_history = d_2.flatten()
#     s_arr_2 = np.hstack((d_1, d_history))
#     s_arr_2 = np.hstack([np.linspace(s_arr_2[i], s_arr_2[i + 1], number_of_increments)
#                          for i in range(len(s_arr_2) - 1)])
#
#     for i in range(len(s_levels.reshape(-1, 2)[:, 1])):
#         print i
#         print len(s_levels)
#         print s_levels.shape
#         print s_levels.reshape(-1, 2)[:, 1].shape
#         print s_levels
#         if i >= len(s_levels.reshape(-1, 2)[:, 1]):
#             break
#         # * a1
#         s_levels.reshape(-1, 2)[i, 0] = s_levels.reshape(-1, 2)[i - 1, 0]
#         # * a2
#         s_levels.reshape(-1, 2)[i, 1] = s_levels.reshape(-1, 2)[i - 1, 1]
#
#     s_history_2 = s_levels.flatten()

#     s_history_2 = [-0, -0.0008, -0.00012, -0.001, -0.0002, -0.0015, -
# 0.00035,  -0.002, -0.0005, -0.003, -0.001, -.004, -0.002, -0.005]
